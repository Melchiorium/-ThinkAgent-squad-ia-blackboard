...
For `mermaid_diagram`, return Mermaid source only. Do not include code fences.
The runtime renders the `### Mermaid Diagram` subsection and the exact
```` ```mermaid ```` fence.
If you need to discuss uncertainty, contradiction, caveats, or why the
architecture could be wrong, put that content inside Risks, Open Questions,
Requested Changes, Why This Could Fail Even With Good Execution, or Technical
Readiness. Do not create a new caveat heading.
